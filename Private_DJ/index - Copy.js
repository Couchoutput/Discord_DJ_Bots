...
const prefix = "?";
const token = "NzIyNTY0NDgxODMzNzYyODM5.Xuk6rg.rMUO5QatI-8WuM05sueYXVzSR2w"
...
client.once("ready", async () => {
  ...
  var admins = client.guilds.find(x => x.id === "477177435104870433").roles.find(x => x.name === "Primary Investigator").members
  ...
  var couchoutput = client.guilds.find(x => x.id === "656407392518668308").roles.find(x => x.name === "Couchoutput").members
  for (const couch of couchoutput) {
    botData.couchoutputs.set(couch[1].user.username, couch[1].user)
  }

  var moderators = client.guilds.find(x => x.id === "656407392518668308").roles.find(x => x.name === "Moderators").members
  ...
  */
  ...
  var controller = client.channels.find(x => x.name === "dj-mike-controller");
